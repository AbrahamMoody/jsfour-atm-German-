...
onNet('jsfour-atm:deposit', ( data ) => {
    ...
    if ( ESX ) {
        ...
        if ( amount <= xPlayer.getMoney() ) {
            ...
            emitNet('esx:showNotification', player, `Du hast eingezahlt: ~s~${ amount }`);
        }
    } else {
        // Otherwise.. Add your own stuff
    }
    ...
    let amount = parseInt(data);
     ...
     if ( ESX ) {
        ...
        if ( amount <= accountMoney ) {
            ...
            emitNet('esx:showNotification', player, `Du hast abgehoben: ~s~${ amount }`);
        }
    } else {
        // Otherwise.. Add your own stuff
    ...
    if ( identifier.length > 0 ) {
        ...
        if ( ESX ) {
            ...
            if ( amount <= accountMoney ) {
                ...
                if ( user.length > 0 ) {
                    emitNet('esx:showNotification', player, `Du hast ~s~${ amount } zu ${ user[0].firstname } überwiesen`);
                    emitNet('esx:showNotification', xReceiver.source, `Du hast ~s~${ amount } von ${ user[1].firstname } erhalten`);
                }
            }
        } else {
            // Otherwise.. Add your own stuff
